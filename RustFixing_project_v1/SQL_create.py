...
def process_and_insert_data(file_path):
    ...
    for index, row in df.iterrows():
        try:
            ...
            #插入数据到数据库中
            cursor.execute("""
                           INSERT INTO query_database(error_description, example_code, solution, solution_code, tag, embedding) 
                           VALUES (%s, %s, %s, %s, %s, %s);
                           """,(error_description, example_code, solution, solution_code, tag, embedding.tolist()))
            # 提交每一条数据的插入
            conn.commit()
            print("insert.........")
...
if __name__ == "__main__":
    file_path = '/home/wyc/save_claude_3.5/tag_knowledge.xlsx'
