...
def slow_process_code(filepath):
    ...
    original_error_count = error_calculate(output)
    print("Begain: ",original_error_count," errors left\n")
   
    if original_error_count != 0:

        solution_file_path = '/home/wyc/save_database_file/solution_saving/code_' + PATH_NAME + '_solution.txt'
        ...
        #solutions circle start
        for line in content:
            #before executing a solution, error_count should be equal to original error count
            ...
            if match:
                ...
                count_step = 0
                start_time = time.time() #record the processing time
                ...
                for step in steps:
                    ...
                    if agent_match:
                        ...
                        output, returncode = run_cargo_miri(code)
                        error_count = error_calculate(output) #after executing a step, error_count will be updated
                ...
                end_time = time.time()
                solution_processing_time = end_time - start_time #calculate processing time
            
                if error_count == 0:
                    successful_count+=1
                    print(f"Solution {solution_num} succeeded! Processing time is {solution_processing_time}s.")
                    print(f"{code}\n")
                    edited_code_folder = '/home/wyc/save_database_file/edited_code_saving/' + PATH_NAME
                    ...
                    #save the passed solution
                    passed_solution_saving_file = '/home/wyc/save_database_file/solution_pass_filter/' + PATH_NAME + '_passed_solution.txt'
                    ...
                    #save solution processing time
                    time_recording_folder = '/home/wyc/save_database_file/time_recording/' + PATH_NAME
                    os.makedirs(time_recording_folder, exist_ok=True)
                    time_recording_file = time_recording_folder + '/' + PATH_NAME + '_time_record'+ str(successful_count) + '.txt'
                    with open(time_recording_file, 'w') as file:
                        file.write(str(solution_processing_time))
        #solutions circle end 

        if successful_count == 0:
            print(f"No solution can solve the error of {PATH_NAME}!")
            fail_code_file = '/home/wyc/save_database_file/failure/' + PATH_NAME + '_failed.rs'
...
def knowledge_modification(result,code,error_message):
    ...
    for key, value in result.items():
        example_code = value['example_code']
        solution = value['solution']
        solution_code = value['solution_code']
        
    prompt = f"1, Q:{example_code}.\
              2, A:{solution_code}.\
              3. Please handle the new code segment and its undefined behavior.\
              4. Important and must:In the format {solution}, the presence of undefined behavior is resolved by assertion or code modification.\
...
def Updata_embedding(embedding_tag,id):
    conn = psycopg2.connect(dbname='rustdb',user='postgres',password='451125',host='localhost',port='5432')
    cur = conn.cursor()
    cur.execute("""
...
def find_similar_errors(embedding):
    ...
    cur.execute(
        """
        SELECT example_code, solution, solution_code 
        FROM query_database
    ...
    res = cur.fetchall()
    result = {}
    for i, (example_code, solution, solution_code) in enumerate(res,1):
        result[f'Prompt{i}'] = {'example_code':example_code, 'solution':solution, 'solution_code':solution_code}
    cur.close()
    conn.close()
    print(result)
    return result
