...
# tell user to input a folder path, and check if the folder path exists or not
while true; do
    echo "Please input a folder path:"
    read folder_path

    if [ -d "$folder_path" ]; then
        # check if there is .rs file in the folder
        rs_files=("$folder_path"/*.rs)
        if [ -e "${rs_files[0]}" ]; then
            cp "$folder_path"/*.rs $HOME/rust_thetis_test/rust_one_trial/
            echo "All .rs files have been copied to rust_one_trial."
            break
        else
            echo "The folder exists, but contains no .rs files. Please try another folder."
        fi
    else
